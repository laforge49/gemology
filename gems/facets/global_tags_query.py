...
def gem_get_gem_full_name(source_gem: Optional[base.Gem], tag_name: str) -> Optional[base.GemFullName]:
...
def resolve_link(source_gem: base.Gem, tag_name: str) -> Optional[base.Gem]:
    full_gemname = gem_get_gem_full_name(source_gem, tag_name)
    if full_gemname is None:
        return None
    if full_gemname == "Aggregate":
        gem_basename = None
        assert isinstance(full_gemname, base.ClusterName)
